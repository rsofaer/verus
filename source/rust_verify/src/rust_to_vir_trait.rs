                    ...
                    match &item.kind {
                        ItemKind::Trait(..) => {
                            ...
                            if let Some((spec, imp)) = vattrs.external_trait_extension {
                                let path = def_id_to_vir_path(tcx, &ctxt.verus_items, trait_def_id, ctxt.name_def_id_map.try_borrow_mut().ok());
...
pub(crate) fn translate_trait<'tcx>(
    ...
    let tcx = ctxt.tcx;
    let orig_trait_path = def_id_to_vir_path(tcx, &ctxt.verus_items, trait_def_id, ctxt.name_def_id_map.try_borrow_mut().ok());
    ...
    if let Some(ex_trait_id_for) = ex_trait_id_for {
        trait_path = def_id_to_vir_path(tcx, &ctxt.verus_items, ex_trait_id_for, ctxt.name_def_id_map.try_borrow_mut().ok());
    }
    ...
    for trait_item_ref in trait_items {
        ...
        let item_path = def_id_to_vir_path(ctxt.tcx, &ctxt.verus_items, owner_id.to_def_id(), ctxt.name_def_id_map.try_borrow_mut().ok());
        let is_verus_spec =
