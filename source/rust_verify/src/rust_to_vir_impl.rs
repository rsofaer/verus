...
fn trait_impl_to_vir<'tcx>(
    ...
    let mut trait_path = def_id_to_vir_path(ctxt.tcx, &ctxt.verus_items, trait_did, ctxt.name_def_id_map.try_borrow_mut().ok());
        ...
        if let rustc_middle::ty::ClauseKind::Trait(_) = inst_pred.kind().skip_binder() {
            ...
            if let Ok(impl_source) = candidate {
                if let rustc_middle::traits::ImplSource::UserDefined(u) = impl_source {
                    let impl_path = def_id_to_vir_path(ctxt.tcx, &ctxt.verus_items, u.impl_def_id, ctxt.name_def_id_map.try_borrow_mut().ok());
...
pub(crate) fn collect_external_trait_impls<'tcx>(
    ...
    for trait_id in all_trait_ids {
        let path = def_id_to_vir_path(tcx, &ctxt.verus_items, trait_id, ctxt.name_def_id_map.try_borrow_mut().ok());
    ...
    'impls: for impl_def_id in auto_import_impls {
        ...
        let impl_path = def_id_to_vir_path(tcx, &ctxt.verus_items, impl_def_id, ctxt.name_def_id_map.try_borrow_mut().ok());
    ...
    for (def_id, span) in external_info.external_fn_specification_trait_method_impls.iter() {
        let trait_method_impl = def_id_to_vir_path(tcx, &ctxt.verus_items, *def_id, ctxt.name_def_id_map.try_borrow_mut().ok());
    ...
    for (impl_path, (impl_def_id, funs)) in new_trait_impls.iter() {
        ...
        let trait_did = trait_ref.skip_binder().def_id;
        let trait_path = def_id_to_vir_path(tcx, &ctxt.verus_items, trait_did, ctxt.name_def_id_map.try_borrow_mut().ok());
        ...
        for (fun_def_id, fun_span) in funs.iter() {
            let path = def_id_to_vir_path(tcx, &ctxt.verus_items, *fun_def_id, ctxt.name_def_id_map.try_borrow_mut().ok());
