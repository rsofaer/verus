...
test_verify_one_file! {
    #[test] test_assume_specification_generics_suggestion_correct code! {
        ...
        verus! {
            pub assume_specification<X, Y, Z> [foo] (_0: X, _1: Y) -> Z
            where
            Y: std::cmp::Eq,
            Z: std::cmp::PartialEq,;
            pub fn bar<X, Y: Eq, Z: PartialEq>(x: X, y: Y) {
...
test_verify_one_file! {
    #[test] test_assume_specification_self_suggestion_correct code! {
        ...
        verus! {
            pub assume_specification [A::foo] (_0: &A) -> A;
            pub fn bar(a: A) {
                let a2 = a.foo();
            }
        }
...
test_verify_one_file! {
    #[test] test_assume_specification_const_generics_suggestion_correct code! {
        ...
        verus! {
            assume_specification<A, const N: usize, const M: usize> [foo] (_0: &[A; N]) -> [A; M];
            pub fn bar<A>(inputs: &[A; 1]) {
                let a = foo::<A, 1, 2>(inputs);
            }
        }
...
test_verify_one_file! {
    #[test] test_assume_specification_region_outlives_correct code! {
        ...
        verus! {
            assume_specification<'a, 'b, 'c, A, B> [foo] (_0: &'a A, _1: &'b B) -> &'c A
            where
            'c: 'a + 'b,;
            pub fn bar<'a, 'b, 'c, A, B>(a: &'a A, b: &'b B) -> &'c A {
                foo(a, b)
...
test_verify_one_file! {
    #[test] external_type_generics_suggestion_correct code! {
        ...
        struct X<'a, A: View + T ,B,C: View<V = A>, const N: usize>
            where B: View<V = C> + T
        {
            a: A,
            b: B,
        ...
        verus! {
            ...
            #[verifier::reject_recursive_types(C)]
            #[verifier::external_type_specification]
            struct ExX<'a, A: View + T,B,C: View<V = A>, const N: usize>(crate::X<'a, A,B,C,N>)
            where B: View<V = C> + T,;

            fn stuff<'a, A: View + T, B, C: View<V = A>, const N: usize>(x: X<'a, A, B, C, N>) -> X<'a, A, B, C, N>
            where B: View<V = C> + T{
                x
            }
        }
